...
export default function Leftbar({ open = true }: { open?: boolean }) {
  return (
    <div
      className={cn(
        "transition-all duration-200",
        open ? "w-60 md:flex" : "w-0 md:w-0 md:flex",
        "hidden md:flex flex-col bg-slate-50 border-r border-slate-200 h-[calc(100vh-3.5rem)] overflow-y-auto"
      )}
      style={{ minWidth: open ? 240 : 0, overflow: open ? undefined : "hidden" }}
    >
      {open && (
        <>
          <div className="p-3">
